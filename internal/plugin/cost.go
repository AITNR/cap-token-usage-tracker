...
func (s *Store) scanCosts(snapshot costQuerySnapshot) (CostResponse, error) {
	...
	err := s.db.View(func(tx *bolt.Tx) error {
		...
		for ; key != nil; key, value = cursor.Next() {
			...
			model := compactModelName(request.Model)
...
func estimateRequestCostWithResolver(request RequestDetail, resolver modelPriceResolver) EstimatedCost {
	price, ok := resolver.resolve(request.Model)
	if !ok && request.Model == "" {
		// Preserve price books configured for the pre-localization placeholder.
		price, ok = resolver.resolve(legacyUntitledModelName)
	}
	if !ok {
